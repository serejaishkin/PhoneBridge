...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        connectionManager = ConnectionManager(tlsClient = TlsClient(), scope = lifecycleScope, endpointStore = endpointStore, peerStore = peerStore, routeStore = routeStore)
        ...
        DiscoveryClient.listen(lifecycleScope) { announce, addr ->
            runOnUiThread {
                val peer = DiscoveredPeer(announce.device_id, announce.device_name, announce.platform, addr, announce.pairing_port, announce.fingerprint)
        ...
        setContent {
            ...
            MaterialTheme {
                Surface(modifier = Modifier.fillMaxSize()) {
                    if (showPairing) {
                        PairingScreen(state = pairingUiState(pairingState), onConfirm = { pairingManager.confirmPairing() }, onReset = { showPairing = false })
                    } else {
                        MainScreen(
                            ...
                            onConnect = { peer ->
                                ...
                                connectionManager.connect(peer, pairingManager.hello(identity.deviceId))
                            },
                            onPairing = { showPairing = true },
                            onConfirmPairing = { pairingManager.confirmPairing() },
                            onForgetPc = { pairingManager.forgetPc(it) },
                            onRequestPermissions = { permissionLauncher.launch(arrayOf(android.Manifest.permission.READ_PHONE_STATE, android.Manifest.permission.ANSWER_PHONE_CALLS, android.Manifest.permission.POST_NOTIFICATIONS)) },
                        )
                    }
                }
            }
    ...
    override fun onDestroy() { connectionManager.disconnect(); callManager.stop(); super.onDestroy() }
}
...
private fun MainScreen(
    ...
    Column(Modifier.fillMaxSize().padding(24.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        if (foundPeers.isEmpty()) Text("No PCs discovered yet. Make sure the phone and PC share a network.")
        else foundPeers.forEach { peer ->
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                if (pairingState is PairingManager.State.Paired && pairingState.pcDeviceId == peer.deviceId) {
                    OutlinedButton(onClick = { onForgetPc(peer.deviceId) }) { Text("Forget") }
                }
            }
        }
        when (val state = pairingState) {
            is PairingManager.State.WaitingForConfirmation -> {
                HorizontalDivider(); Text("Pairing code: ${state.shortCode}", style = MaterialTheme.typography.titleMedium)
                Text("Compare this code with the PC before confirming.")
                Button(onClick = onConfirmPairing) { Text("Confirm pairing") }
            }
            is PairingManager.State.Paired -> { HorizontalDivider(); Text("PC paired: ${state.pcDeviceId}") }
            is PairingManager.State.Failed -> { HorizontalDivider(); Text("Pairing error: ${state.message}") }
        ...
        OnboardingStep.ORDER.forEach { step -> Text("• ${step.title}: ${step.explanation}", style = MaterialTheme.typography.bodySmall) }
    }
}

private fun pairingUiState(state: PairingManager.State): com.phonebridge2.app.ui.PairingUiState = when (state) {
    is PairingManager.State.WaitingForConfirmation -> com.phonebridge2.app.ui.PairingUiState(peerName = state.pcDeviceId, code = state.shortCode, step = com.phonebridge2.app.ui.PairingStep.ConfirmCode)
    is PairingManager.State.Paired -> com.phonebridge2.app.ui.PairingUiState(peerName = state.pcDeviceId, step = com.phonebridge2.app.ui.PairingStep.Complete)
