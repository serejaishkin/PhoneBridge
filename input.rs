...
impl AudioInput {
    pub fn new() -> Result<(Self, Sender<Vec<u8>>), Box<dyn std::error::Error>> {
        ...
        let stream = match config.sample_format() {
            cpal::SampleFormat::I16 => Self::build_stream::<i16>(&device, &config.into(), pcm_tx_clone)?,
            cpal::SampleFormat::F32 => Self::build_stream::<f32>(&device, &config.into(), pcm_tx_clone)?,
    ...
    fn build_stream<T: cpal::SizedSample + Into<i16> + Send + 'static>(
        device: &cpal::Device,
        ...
        device.build_input_stream(
            ...
            move |data: &[T], _: &cpal::InputCallbackInfo| {
                ...
                for chunk in data.chunks(channels) {
                    let sum: i32 = chunk.iter().map(|s| s.into() as i32).sum();
                    mono.push((sum / channels as i32) as i16);
                }
                // Buffer 20ms frames (960 samples @ 48kHz)
                // For simplicity we send every chunk; real impl needs framing
