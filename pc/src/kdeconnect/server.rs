...
async fn handle_connection(
    ...
    write_packet(&mut stream, &Packet::identity(0, &local)?).await?;

    let mut peer_identity: Option<IdentityPacket> = None;
    ...
    loop {
        ...
        match packet.packet_type.as_str() {
            ...
            "kdeconnect.pair" => {
                ...
                if !pair.pair {
                    // A false pair packet is a rejection/unpair request and
                    // does not require a response.
                    return Ok(());
                }

                let identity = peer_identity.clone().ok_or_else(|| {
                    anyhow::anyhow!("received kdeconnect.pair before kdeconnect.identity")
                })?;
