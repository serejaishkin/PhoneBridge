...
impl HfpBackend for UnsupportedHfpBackend {
    ...
    fn answer_call(&self) -> Result<(), String> {
        Err("HFP backend is not available on this platform/build".into())
    }
...
pub mod platform {
    ...
    impl HfpBackend for WindowsHfpBackend {
        ...
        fn answer_call(&self) -> Result<(), String> { Err("Windows HFP integration pending".into()) }
        fn decline_call(&self) -> Result<(), String> { Err("Windows HFP integration pending".into()) }
        fn end_call(&self) -> Result<(), String> { Err("Windows HFP integration pending".into()) }
    }
}
...
pub fn create_backend() -> Box<dyn HfpBackend> {
    #[cfg(target_os = "windows")]
    { return Box::new(platform::WindowsHfpBackend); }
    #[cfg(target_os = "linux")]
